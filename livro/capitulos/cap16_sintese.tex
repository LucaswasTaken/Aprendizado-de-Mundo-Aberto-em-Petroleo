...
No primeiro, a avaliação publicada do sistema dual --- regras e autoencoder --- reportou
operação em mais de duzentos e cinquenta poços e estudos de caso com eventos fora do
catálogo. O sistema detectava desvios, mas não sabia dizer o que via.
...
\begin{destaque}[Ganhos grandes vieram de mudanças conceituais simples]
Trocar o autoencoder pelo latente do classificador multiclasse elevou a detecção de
novidade em quarenta e seis pontos percentuais neste protocolo. A alteração de inferência
é pequena, mas depende do custo anterior de treinar e validar o classificador.
...
novidade arquitetural --- a U-Net é de 2015 --- e aumentou o custo de anotação por
amostra.
...
\textbf{Por coluna}: nas avaliações apresentadas, reconhecer incompatibilidade obteve
taxas maiores que afirmar identidade para \emph{ambas} as famílias de método. Uma
explicação plausível é que uma evidência contraditória basta para descartar uma hipótese,
enquanto afirmar identidade exige convergência; protocolos e amostras diferentes,
contudo, impedem tratar a diferença bruta como comparação causal.
...
\addlinespace
7 & Incrustação & Fraco ($52{,}3\,\%$) & \textbf{Segunda opinião priorizada}; rejeitou $100\,\%$ dos erros na amostra \\
\addlinespace
...
O percurso deste livro sugere sete princípios para teste além do problema de poços
submarinos. Alguns decorrem da formulação matemática; outros são generalizações
empíricas que ainda exigem validação em novos domínios.
...
\item \textbf{Reconhecer incompatibilidade pode ser mais fácil que afirmar identidade.}
Essa assimetria foi observada nos métodos geométricos e no modelo de linguagem avaliados.
Projete sistemas que a meçam e, quando confirmada, a explorem.
...
\item \textbf{Reduzir a variância pode ser tão importante quanto aumentar a média.}
Entre sistemas com acurácia média de $0{,}86$, aquele com desvio $0{,}08$ é mais
previsível entre poços do que outro com desvio $0{,}19$. Isso favorece a avaliação
operacional, mas não basta para declarar nenhum deles implantável.
...
\item \textbf{Um sistema que descreve o que vê pode ajudar a auditar os dados.} Nesta
análise, a hipótese de artefato de telemetria ganhou visibilidade quando quatro classes
receberam nomes contendo ``perda de telemetria''; as matrizes de confusão examinadas não
a haviam evidenciado.
...
decidem nas classes 1, 2, 3, 6, 8 e 9; o agente fornece segunda opinião priorizada na
classe 7, onde o
...
\item \textbf{Projeto integrador.} Especifique e implemente uma versão da arquitetura de
      referência de oito camadas e meça o desempenho fim a fim sobre o 3W. Como os
      artefatos completos dos estudos originais não acompanham esta edição, registre as
      decisões que precisou reconstruir. Compare com cada componente isolado. O ganho da
      integração é aditivo, subaditivo ou superaditivo?

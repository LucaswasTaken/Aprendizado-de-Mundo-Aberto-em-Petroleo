...
Sob normalidade aproximada, a probabilidade unilateral de o escore ultrapassar $\tau$
é de aproximadamente $0{,}135\,\%$. O valor de cerca de $0{,}27\,\%$, frequentemente
associado à regra dos três desvios, soma as duas caudas e não se aplica a este limiar
unilateral. Como a distribuição do erro de reconstrução não é gaussiana, porém, esses
valores são apenas referências teóricas: a taxa de falso positivo relevante deve ser
medida empiricamente no conjunto de validação.
...
Os cortes de $30\,\%$ e $70\,\%$ são regras operacionais de triagem adotadas no sistema,
não probabilidades calibradas de ocorrência. Sua transferência para outro poço requer
calibração com a matriz de custo e a taxa de alarmes observada localmente.
...
À época da avaliação, \citet{aranha2023system} reportam que o sistema estava implantado
em \textbf{mais de vinte unidades flutuantes de produção} e \textbf{mais de duzentos e
cinquenta poços submarinos} nas bacias de Santos e de Campos. Esta edição não auditou
independentemente a escala dessa implantação.
...
percentual parece pouco. A redução relativa de $61\,\%$ descreve as taxas agregadas; sem
contagens pareadas e protocolo operacional, não permite afirmar que três de cada cinco
erros individuais deixaram de ocorrer.
...
\item \textbf{Projeto de extensão.} Com acesso ao 3W e a um ambiente de aprendizado
      profundo, implemente o autoencoder descrito neste capítulo e treine-o sobre
      instâncias normais. Compare o histograma do SMAPE de reconstrução em dados normais e

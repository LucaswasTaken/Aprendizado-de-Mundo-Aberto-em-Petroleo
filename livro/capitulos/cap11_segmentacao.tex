...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  \item distinguir detecção de segmentação e justificar por que a segunda fornece
        informação temporal mais útil ao operador;
  \item descrever o projeto de uma U-Net unidimensional para segmentação de séries
        temporais;
...
melhor que o caso médio anterior. Essa propriedade --- e não apenas a média --- reduz
um obstáculo à implantação; não demonstra, por si só, prontidão operacional.
\end{destaque}
...
passar cerca de um terço dos eventos. A preferência entre esse perfil e outro com mais
falsos alarmes depende do custo e da criticidade de cada classe; não pode ser definida
apenas pelas métricas agregadas. A revocação de $0{,}68$ é uma limitação relevante.
...
A segunda hipótese é plausível e aponta para possível \emph{desalinhamento da anotação}
do poço A3. A alta acurácia de segmentação é compatível com essa leitura, mas uma revisão
cega por especialista é necessária para distingui-la de erro de implementação da métrica
ou comportamento espúrio do modelo.
\end{destaque}
...
$0{,}187$ para $0{,}082$ --- a menor dispersão favorece a generalização entre os poços
avaliados, mas implantação requer validação prospectiva, latência e análise de falhas.
A interseção sobre união global é de $85\,\%$. A avaliação
...
\item \textbf{Projeto de reprodução e ablação.} Com acesso aos dados, implemente a U-Net
      e busque reproduzir a \cref{tab:seg-comparacao}, registrando divisão, sementes e
      divergências. Em seguida, remova as conexões de atalho e repita. Quantifique a
      degradação em acurácia e, separadamente, em IoU. Qual das duas métricas é mais
      sensível à remoção? Por quê?

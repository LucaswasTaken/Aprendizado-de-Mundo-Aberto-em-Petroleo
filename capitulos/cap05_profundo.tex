...
procedimento preserva a ordem apenas de forma posicional: cada instante ocupa
coordenadas fixas, de modo que permutá-los em geral altera a saída. O que falta é o
viés arquitetural adequado à estrutura temporal: não há compartilhamento local de
pesos nem equivariância a translações, e o mesmo padrão em posições diferentes ativa
parâmetros diferentes. Além disso, o número de pesos da primeira camada cresce com
$T \cdot S$.
...
diferença observada é modesta e, sem uma análise pareada, não estabelece superioridade
estatística. O fato de a representação bruta chegar tão perto ainda é informativo: as
assinaturas dessas anomalias são, em boa medida, capturáveis por
...
Partindo de campo $r=1$ e salto $j=1$, cada convolução atualiza
$r\leftarrow r+4j$; cada \ing{pooling} de núcleo e passo 2 atualiza
$r\leftarrow r+j$ e $j\leftarrow2j$. Os valores após os dois blocos são
$(r,j)=(28,4)$; as duas convoluções do gargalo levam a $r=60$ amostras. O valor depende
da convenção de borda, mas não ultrapassa oitenta nessa arquitetura.

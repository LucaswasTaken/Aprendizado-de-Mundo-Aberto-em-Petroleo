...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  \item projetar uma cadeia de pré-processamento que produza sinais comparáveis entre
        poços;
  \item calcular as treze métricas por sensor que traduzem um segmento anômalo em
        descrição quantitativa;
  \item construir perfis estatísticos de classe robustos a valores extremos;
  \item montar o prompt de um agente de diagnóstico e validar sua saída;
...
\item \textbf{Padronização por escore-$z$.} Cada sensor é centrado e escalado pela sua
própria média e desvio padrão. Sem isso, pressões de centenas de bar dominariam
temperaturas de dezenas de graus em qualquer comparação.

\item \textbf{Normalização Min-Max para $[0,1]$.} Aplicada após a padronização, garante
faixa comparável entre poços de níveis operacionais distintos.
...
de fronteira. O pré-processamento aplica padronização, normalização Min-Max, remoção de
...
\begin{exercicios}
\item Implemente as treze métricas e aplique-as a instâncias de cada classe do 3W.
...
\item Escreva o prompt completo descrito neste capítulo e submeta o mesmo segmento vinte
      vezes com temperatura $0{,}2$. Meça a variabilidade da saída. Repita com

...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  \item explicar a diferença estrutural entre um classificador multiclasse e um ensemble
        ...
        expressar rejeição;
  \item descrever o projeto e o treinamento de um classificador binário por classe de
        anomalia, incluindo o tratamento do desbalanceamento da classe negativa;
  \item interpretar os resultados de uma busca sistemática de hiperparâmetros;
...
ponto percentual; sem as predições pareadas ou um teste de não inferioridade, não é
possível afirmar superioridade nem equivalência.
\end{resultadochave}
...
\textbf{a capacidade de rejeição foi acrescentada sem perda observada na acurácia
agregada} neste experimento --- um resultado compatível com o objetivo, mas que não
demonstra não inferioridade.

A leitura otimista seria: ``o ensemble é superior''. Um ponto percentual sobre esse
conjunto não sustenta essa afirmação.

\begin{destaque}[O que a tabela realmente sustenta]
Que a acurácia agregada de mundo fechado permaneceu próxima à do classificador
multiclasse enquanto a arquitetura passou a expressar rejeição. A evidência de que essa
rejeição funciona para uma classe omitida está no \cref{cap:closedworld}, no experimento
em que a classe 8 é escondida e $89\,\%$ de suas instâncias são sinalizadas como
desconhecidas.
\end{destaque}
...
classe 8. Essa associação é pós-hoc e deve ser testada com medidas de separabilidade ou
ablação. Uma explicação plausível é que, quando a assinatura é distintiva, um modelo dedicado a
...
Se a taxonomia for internamente inconsistente, mudar apenas a arquitetura não resolve o
problema. As classes 4 e 5 tendem a
...
As duas últimas colunas da \cref{tab:binarios-porclasse} servem apenas como contexto e
não como comparação controlada. Os valores
binários de \citet{Marins2021} --- $0{,}97$ a $1{,}00$ --- são superiores aos deste
capítulo em todas as classes. Três diferenças de protocolo podem contribuir para a
discrepância:
...
aplicada ao espaço bruto: não houve perda observada na acurácia agregada, embora o
experimento não estabeleça não inferioridade. A
...
ambas as abordagens, resultado compatível com uma limitação da taxonomia, mas que não
exclui limitações dos modelos.}
...
\item \textbf{Projeto de reprodução.} Com acesso ao 3W, implemente o ensemble e busque
      reproduzir a \cref{tab:binarios-porclasse}. Registre versão dos dados, divisão,
      sementes e divergências em relação aos valores publicados. Em seguida, repita o
      experimento com divisão aleatória de janelas em vez de divisão temporal e

...
Ao excluir verdadeiros negativos, o IoU mede diretamente a sobreposição da classe
positiva e não é inflado pela classe majoritária. Isso não o torna universalmente
``mais exigente''; ele responde a uma pergunta diferente da acurácia por amostra. É
...
\begin{exemplo}{Intervalos individuais não testam uma diferença}
...
segundo método seria aproximadamente $[33{,}3\,\%;\ 47{,}1\,\%]$. Esses dois
intervalos descrevem cada proporção separadamente; sua sobreposição, ou ausência dela,
não é um teste da diferença entre os métodos.

Como os dois métodos foram avaliados nos mesmos segmentos, a comparação é pareada. Para
acerto e erro binários, o teste de McNemar usa apenas os casos discordantes: $b$, em que
somente o primeiro acerta, e $c$, em que somente o segundo acerta. Para métricas como
$F_1$ ou acurácia balanceada, um intervalo da diferença pode ser obtido por
\ing{bootstrap} pareado, reamostrando segmentos ou poços como unidades indivisíveis.
Se a hipótese for ``não houve perda relevante'', é necessário definir antes uma margem
de não inferioridade; ausência de significância não demonstra equivalência.
...
Toda proporção reportada com $n < 200$ deve vir acompanhada de $n$ e de intervalo de
$95\,\%$ quando as contagens estiverem disponíveis. Toda média sobre poços ou dobras
deve vir acompanhada de uma medida de dispersão. Comparações sobre os mesmos casos devem
preservar o pareamento; intervalos individuais não substituem um teste ou intervalo da
diferença. Quando a dispersão é grande em relação à média --- como o
$0{,}34 \pm 0{,}42$ do \cref{cap:mundoaberto} --- o texto deve discutir explicitamente

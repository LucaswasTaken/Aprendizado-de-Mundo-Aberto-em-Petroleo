...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  ...
  \item explicar por que, nos experimentos apresentados, uma representação treinada
        para discriminar supera uma treinada para reconstruir na detecção do desconhecido;
...
A interpretação oferecida na ocasião foi que o gargalo estava em outro lugar. Os
experimentos deste capítulo sustentam uma explicação mais específica para este conjunto:
o problema não é \emph{qual} autoencoder, mas usar uma representação otimizada para
reconstrução na tarefa de separar classes.
...
A hipótese testada é que \textbf{esse espaço, construído para discriminar entre classes
conhecidas, também é melhor para reconhecer o desconhecido neste protocolo} --- mesmo
que nada em seu treinamento tenha visado a isso.
\end{destaque}
...
um ganho concentrado em poucos casos favoráveis: é um padrão consistente neste
protocolo. Essa consistência, sozinha, não exclui seleção de hiperparâmetros, vazamento ou
ajuste ao conjunto; uma ablação controlada deve separar objetivo de treino, arquitetura
e dimensão da representação.
...
Essa é uma hipótese física plausível, a ser testada variando o horizonte; o resultado
atual não exclui limitações da representação ou da amostra.
...
\item \textbf{Possível confundimento de telemetria.} As representações e os limiares
foram aprendidos sobre dados em que valores ausentes, zerados ou congelados podem
coincidir com os rótulos. Sem a ablação proposta no \cref{cap:futuro}, os resultados
medem o protocolo observado, mas não isolam quanto da separação decorre da assinatura
física e quanto decorre do processo de aquisição.
...
\item \textbf{Projeto de reprodução e extensão.} Com acesso aos dados, implemente os três
      espaços e busque reproduzir a \cref{tab:mahal-qualidade}, registrando protocolo e
      divergências. Em seguida, calcule a silhueta do latente de um classificador
      multiclasse treinado com diferentes números de classes ($3$, $5$, $7$, $9$). A
      qualidade do espaço melhora monotonicamente com o número de classes?

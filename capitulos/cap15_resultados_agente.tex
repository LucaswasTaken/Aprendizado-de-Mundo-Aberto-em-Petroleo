...
evidência mais forte, neste conjunto de dados, de que a taxonomia contribui para a
dificuldade; ela não isola essa causa de limitações de representação ou amostragem.
...
em mais da metade o volume que exige atenção detalhada nesta amostra, na qual
aproximadamente $9\,\%$ das confirmações foram incorretas. Antes do uso operacional,
essa taxa precisa ser estimada prospectivamente e comparada ao custo dos erros. É uma
proposta de triagem mensurável, diferente de ``a IA classifica os eventos''.
\end{destaque}
...
\begin{table}[htbp]
\centering
\caption{Taxa de reconhecimento de novidade pelo agente nas sete classes não
sintomáticas avaliadas.}
...
9 & $6/11$    & $54{,}5\,\%$ \\
\midrule
\textbf{Global (7 classes)} & $\bm{548/611}$ & $\bm{89{,}7\,\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{resultadochave}
Nas sete classes não sintomáticas, o agente reconhece corretamente que um segmento não
pertence a nenhuma das classes apresentadas em $\bm{89{,}7\,\%}$ dos casos --- contra
$35{,}1\,\%$ de acerto quando
...
Isso levanta a hipótese de que reconhecer incompatibilidade seja mais fácil que
estabelecer identidade: uma evidência contraditória pode descartar uma classe, enquanto
afirmar identidade exige convergência. Como tarefas, amostras e denominadores diferem, a
comparação não demonstra que a assimetria seja propriedade universal do problema.
\end{destaque}
...
Três observações tornam a memorização pura uma explicação incompleta. Primeiro, se ela
dominasse o resultado, seria razoável esperar acurácia de classificação alta, não
$35{,}1\,\%$. Segundo, os erros seguem
...
Essas observações não medem contaminação nem excluem sua contribuição. Sustentam apenas
que ela, isoladamente, não explica todos os padrões observados.
\end{destaque}
...
um padrão a testar em avaliações controladas. Os nomes gerados convergem para descrições
consideradas fisicamente plausíveis na análise qualitativa
...
``perda de telemetria'', levantando a hipótese de um artefato do 3W forte o bastante para
dominar a descrição e exigir ressalva sobre os resultados que usam esse conjunto.}

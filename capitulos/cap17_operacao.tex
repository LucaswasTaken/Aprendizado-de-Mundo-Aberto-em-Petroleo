...
A maior parte dos números deste livro foi obtida em avaliação controlada. O
\cref{cap:dual} também apresenta estudos de caso de campo e uma escala de implantação
reportada pela fonte primária, superior a vinte unidades flutuantes; esta edição não
auditou independentemente essa implantação. É justamente por isso que ele é o capítulo
com o menor número de tabelas e o maior número de decisões de engenharia aparentemente
banais.
...
Uma estimativa de custo total de propriedade precisa fixar um horizonte $H$ e separar
custos fixos, computação, chamadas e revisão humana. Um modelo mínimo é
\begin{equation}
      C_{\mathrm{TCO}}(H) = C_{\mathrm{implantação}} + C_{\mathrm{infra}}(H)
      + \frac{N(H)}{10^6}\bigl(T_{\mathrm{in}}p_{\mathrm{in}}
      + T_{\mathrm{out}}p_{\mathrm{out}}\bigr)
      + N(H)r\,t_h c_h + C_{\mathrm{armazenamento}}(H),
      \label{eq:oper-tco}
\end{equation}
onde $N(H)$ é o número de segmentos enviados ao agente, $T_{\mathrm{in}}$ e
$T_{\mathrm{out}}$ são os \ing{tokens} médios por chamada, $p_{\mathrm{in}}$ e
$p_{\mathrm{out}}$ são os preços contratados por milhão de \ing{tokens}, $r$ é a
fração encaminhada à revisão, $t_h$ é o tempo de revisão em horas e $c_h$ é o custo da
hora do especialista.

\begin{exemplo}{Dimensionamento mensal sem congelar preços comerciais}
Com cinquenta segmentos por dia durante trinta dias, \num{6000} \ing{tokens} de entrada
e \num{1000} de saída por segmento, o consumo mensal é de nove milhões de \ing{tokens}
de entrada e $1{,}5$ milhão de saída. O custo da API é, portanto,
$9p_{\mathrm{in}}+1{,}5p_{\mathrm{out}}$. Se $41\,\%$ dos casos exigirem dez minutos de
revisão, serão $102{,}5$ horas de especialista, com custo $102{,}5c_h$. Substituir na
\cref{eq:oper-tco} os preços do contrato, a infraestrutura e o armazenamento produz a
estimativa mensal; variar $N$, $r$ e $t_h$ revela qual termo domina.
\end{exemplo}

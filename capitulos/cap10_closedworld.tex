...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  \item conduzir e interpretar um experimento de mundo aberto por classe escondida;
  \item analisar o compromisso entre comprimento de janela, acurácia e tempo de resposta
        operacional;
  \item validar um classificador contra um modelo físico independente e interpretar as
        discrepâncias;
  \item estimar o custo computacional de um ensemble e avaliar sua viabilidade em campo;
...
\emph{pode} rejeitar, e mostrou que essa capacidade não custa acurácia. Faltava a parte
difícil: demonstrar que ela \emph{funciona} quando confrontada com uma classe que o
sistema realmente nunca viu.

Este capítulo apresenta essa demonstração, junto com três investigações que a
...
$\sum_k \hat{p}_k = 1$ não lhe permite outra resposta.}
...
Quanto tempo de sinal é necessário para reconhecer um hidrato? A pergunta parece
técnica, mas sua resposta é operacional: cada segundo de janela é um segundo de atraso
na detecção.
...
\begin{table}[htbp]
\centering
\caption{Compromisso entre tempo de resposta e acurácia para três comprimentos de janela
representativos.}
\label{tab:closed-tempo}
\begin{tabular}{rcc}
\toprule
$\bm{T}$ \textbf{(s)} & \textbf{Tempo de resposta} & \textbf{Acurácia} \\
\midrule
...
minutos e meio de atraso --- custa apenas $3{,}6$ pontos percentuais de acurácia.
\end{resultadochave}

Esse número precisa ser confrontado com a física do problema. Segundo \citet{VARGAS2019},
o tempo característico de resposta a um evento de hidrato varia de \textbf{trinta
minutos a cinco horas}. Ou seja: mesmo a configuração mais lenta da tabela detecta o
evento em uma fração do tempo disponível para agir.
...
binário --- cujos resultados são então confrontados antes de gerar o alerta.}
\label{fig:closed-arquitetura}
\end{figure}

A correlação de \citet{montie1991} estima a temperatura de formação de hidrato
$T_{\text{hid}}$ a partir da pressão $P$ e da densidade relativa do gás $q$:
\begin{equation}
\begin{split}
  T_{\text{hid}} = {} & -238{,}245 + 78{,}99\,P - 5{,}35\,P^2 \\
                      & + 349{,}4738\,q - 151{,}05\,q^2 - 27{,}60\,Pq.
\end{split}
\label{eq:motiee}
\end{equation}
...
esquerda da curva colocam o sistema em risco.}
...
Considere um trecho de linha operando a $P = \SI{7}{\mega\pascal}$ com gás de densidade
relativa $q = 0{,}65$ e temperatura medida $T = \SI{14}{\celsius}$.

Aplicando a \cref{eq:motiee}, obtém-se uma temperatura de formação
$T_{\text{hid}}$ da ordem de \SI{12}{\celsius}. Pela \cref{eq:chid},
$C_{\text{hid}} \approx 100 \times (14 - 12)/14 \approx 14\,\%$ --- ou seja, o sistema
opera com margem térmica de aproximadamente catorze por cento acima da condição de
formação.

Uma queda de dois graus na temperatura da linha, perfeitamente possível durante uma
parada, levaria $C_{\text{hid}}$ a zero. É esse tipo de raciocínio de margem que o
modelo analítico oferece e que um classificador, por si só, não comunica.
\end{exemplo}
...
\SI{405}{\milli\second} --- para um evento cujo tempo característico de resposta é de
trinta minutos a cinco horas.
\end{resultadochave}

A margem é de três a quatro ordens de grandeza. Cabe em um computador industrial
embarcado, e o custo de multiplicar por nove --- que parecia uma objeção séria no
\cref{cap:binarios} --- revela-se irrelevante.

\begin{destaque}[Uma observação sobre argumentos de eficiência]
Este é um caso em que a quantificação encerra o debate. Objeções de custo computacional
devem ser sempre confrontadas com a escala de tempo do processo monitorado. Em
monitoramento de poços, com constantes de tempo de minutos a horas, praticamente
qualquer modelo que caiba na memória disponível é rápido o bastante. A engenharia deve
ser gasta em outro lugar.
\end{destaque}
...
minutos e meio custa apenas $3{,}6$ pontos percentuais --- irrelevante frente ao tempo
característico de resposta ao hidrato, de trinta minutos a cinco horas. O gargalo,
portanto, não é a latência, mas a confiabilidade. A validação contra a correlação
...
menor que a escala de tempo do processo, encerrando a objeção de eficiência.}
...
\item Implemente a \cref{eq:motiee} e a \cref{eq:chid} e aplique-as às instâncias das
      classes 8 e 9 do 3W. Reproduza a \cref{tab:closed-analitico}. Em seguida, examine
      os casos em que o critério analítico erra e o classificador acerta: há padrão?
...
\item \citet{VARGAS2019} --- a descrição do 3W e dos tempos característicos de cada
      evento.
